...
for i in range(0,10):
    x_train, x_test, y_train, y_test = train_test_split(Xs[i], ys[i], test_size=1./3, random_state=133)

## todo 2.a
    
    for j in range(0,15):
        
        tclf = DecisionTreeClassifier(max_depth=j+1, random_state=330)

        tclf = tclf.fit(x_train, y_train)
        
        y_test_pred = tclf.predict(x_test);
                                                                                ## todo 2.c, 2.d
        test_ac[i][j] = accuracy_score(y_test, y_test_pred)    
        
        y_train_pred = tclf.predict(x_train)
        
        train_ac[i][j] = accuracy_score(y_train, y_train_pred)    
